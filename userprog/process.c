...
/* Gathers parameters from the command line.
   TODO: Where to put this? Most likely in
   {process.c : bool load}
*/
void *
push_parameters (char *argv)
{
  ...
  for (token = strtok_r(argv, " ", &save_ptr);
    ...
    bash = (struct command *)malloc(sizeof(struct command));
    bash->word = token;
    num_paramaters++;
   ...
   * paramater and place it on a temp stack */
